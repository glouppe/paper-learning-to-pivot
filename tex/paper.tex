...
\begin{figure*}
    ...
    \vspace{-1cm}
    \caption{Toy example.
    (Upper left) Conditional probability densities of the decision scores for $Z=0, 1, 2$
    ...
    (Lower left) Conditional probability densities of the decision scores for $Z=0, 1, 2$ when $f$ is
       built with adversarial training, as outlined in Section~\ref{sec:method}.
       The resulting densities are now much closer to each other, indicating only a
...
\begin{figure}
    ...
    \caption{Toy example. Training curves for ${\cal L}_f(\theta_f)$, ${\cal L}_r(\theta_f, \theta_r)$
             ...
             Adversarial training was performed for $300$ iterations, mini-batches of size $M=128$, $K=10$ and $\lambda=10$.}
...
\begin{align}
    x &\sim {\cal N}(\mu=(0,0), \Sigma= \mathbb{1}) \text{ when } Y=0, \\
    x &\sim {\cal N}(\mu=(1,z), \Sigma= \mathbb{1}) \text{ when } Y=1.
\end{align}
...
By contrast, when running Algorithm~\ref{alg:adversarial-training} initialized
from the classifier obtained previously, in combination with a mixture density
network $r$ (modeling $Y|f(X;\theta_f)$ as a truncated mixture of four gaussian
components), adversarial training effectively reshapes the decision function
$f$, so it that becomes almost independent on $Z$, as shown in the lower plots
of Figure~\ref{fig:toy}. In particular, the conditional probability densities of
...
making the classifier less optimal, hence the corresponding increase of ${\cal
...
goes, minimizing $E$ requires making predictions that are more accurate, hence
decreasing ${\cal L}_f$, or that are less dependent on $Z$, hence shaping
$p_{\theta_r}$ so that it becomes more uniform. Indeed, ${\cal L}_f$ eventually
start to slighlty decrease, while remaining lower bounded by $\min_{\theta_f}
