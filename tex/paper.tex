...
nuisance parameters that parametrizes this family of data generation processes. In
this work, we introduce a flexible adversarial training procedure to enforce the pivotal
property on a predictive model (e.g., a classifier), which allows one to tune the trade-off between
...
$f(X ; \theta_f)$ conditional on $Z$ (and possibly $Y$) be invariant with
 the nuisance parameter $Z$. Thus, we wish find a predictive function $f$ such that
...
optimal and pivotal, provided such a classifier exists. Let us note however
...
We reuse the datasets used in reference
...
the hyperparameter $\lambda$ of Eqn.~\ref{eqn:vf-lambda} with respect to
...
wish to impose the pivotal property on $Y=0$ events. Furthermore,
...
gives us a principled and effective approach to control the tradeoff between
...
In practice, parametrized classifiers  are  computationally expensive to build
...
Section~\ref{sec:theory}, but that can otherwise be directly used in a wider range of
...
classification. Again, trying our algorithm on these use cases is certaintly
worth investigating.
