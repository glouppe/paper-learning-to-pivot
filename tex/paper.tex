...
\begin{figure*}
    ...
    \vspace{-1cm}
    \caption{Toy example.
    (Top left) Conditional probability densities of the decision scores for $Z=0$, $2.5$ and $5$
    ...
    (Top right) The corresponding decision surface, highlighting
       the fact that samples are easier to classify for values of $Z$ closer to $5$,
       hence explaining the dependency.
    (Bottom left) Conditional probability densities of the decision scores for $Z=0$, $2.5$ and $5$ when $f$ is
       built with adversarial training, as outlined in Section~\ref{sec:method}.
       The resulting densities are now closer to each other, indicating only a
       small dependency on $Z$.
    (Bottom right) The corresponding decision surface, illustrating how adversarial
...
\begin{figure}
    ...
    \vspace{-1cm}
    \caption{Toy example. Training curves for $L_f(\theta_f)$, $L_r(\theta_f, \theta_r)$
             and $L_f(\theta_f) - \lambda L_r(\theta_f, \theta_r)$.
             Adversarial training was performed for $1000$ iterations, mini-batches of size $M=128$, $K=100$ and $\lambda=50$.}
    \label{fig:toy-training}
\end{figure}

As a guiding toy example, let us consider the binary classification problem of 2D
...
Assuming a uniform prior over the interval ${\cal Z} = [0;5]$, we start by
generating training data $\{ x_i, y_i, z_i \}_{i=1}^N$, from which we build a
neural network classifier $f$ minimizing $L_f(\theta_f)$ without adversarial
training. As shown in the first row of Figure~\ref{fig:toy}, the resulting
...
becomes effectively less dependent on $Z$, as shown in the second row of
Figure~\ref{fig:toy}. In particular, the conditional probability densities of
the decision scores $f(X;\theta_f)$ are now closer to each other, indicating only a
small  dependency on the nuisance, as theoretically expected.
\glnote{Discuss Figure~\ref{fig:toy-training}}
