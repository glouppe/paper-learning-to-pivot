...
\theta_r)$, in which case $p_{Z|f(X;\theta_f)} =
p_{r(f(X;\theta_f);\hat\theta_r)|f(X)}$ and $L_r$ reduces to the entropy
...
holds at $\smash{\hat\theta}_f$ when
\begin{itemize}
...
point, because of Proposition~\ref{prop:1}, $(\smash{\hat\theta}_f, \smash{\hat\theta}_r)$, $f$ is both an optimal
classifier and a pivotal quantity.
\end{proof}
