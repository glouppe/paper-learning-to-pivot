...
\begin{abstract}
Systematic uncertainties in high energy physics, representing
incomplete knowledge in the simulation of physical processes or
detection appartus, may cause variations in the class conditional
distributions of features used for training a classifier.  As such,
the efficiacy of a classifier may change under the variations caused
by systematic uncertainties.  In this paper, we propose an adversarial
training procedure to produce a pivotal
classifier which is invariant to variations of nuisance parameters
which parameterize the effects of systematic uncertainties.
In the machine learning context, this problem is closely related to those of domain
adaptation and enforcing fairness in classification.  The techniques
described here can handle the case of continuous nuisance paramater
variations that represents domain adaptation over infinitely many domains. 
...
\begin{figure}
    ...
    \caption{Toy example. Training curves for ${\cal L}_f(\theta_f)$, ${\cal L}_r(\theta_f, \theta_r)$
             ...
             Adversarial training was performed for $200$ iterations, mini-batches of size $M=128$, $K=100$ and $\lambda=50$.}
...
truncated mixture of four gaussians over ${\cal Z}$. When running
