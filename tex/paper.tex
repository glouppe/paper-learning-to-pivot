...
with respect to $\lambda$. Note that a function $f$ for which
...
sets of samples $\{ \omega | f(X_{\lambda_i}(\omega); \theta_f) = y \}$ and $\{
\omega' | f(X_{\lambda_j}(\omega'); \theta_f) = y \}$ do not need to be the same
for the equality to hold.
With this distinction, but as only Eqn.~\ref{eqn:criterion-measure} is
of direct interest in this work, we further simplify notations by denoting
the pivotal quantity criterion as
\begin{equation}\label{eqn:criterion}
    p(f(X ; \theta_f) | \lambda_i ) = p(f(X ; \theta_f) | \lambda_j ) \text{ for all $\lambda_i,\lambda_j \in  \Lambda$}
\end{equation}.
...
associated loss ${\cal L}_r(\theta_r)$. This model takes as input realizations of $f(X; \theta_f)$
and produces as output estimates $r(f(X; \theta_f)) = \hat{p}(\lambda | f(X; \theta_f)) $
that $f(X; \theta_f)$ is generated from the nuisance value $\lambda$. If $p(f(X; \theta_f)|\lambda)$ varies with $\lambda$,
...
\glnote{Sketch the optimization algoritm}
