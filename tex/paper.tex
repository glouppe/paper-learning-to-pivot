...
\begin{algorithmic}[1]
    \For{$t=1$ to $T$}
        \For{$k=1$ to $K$} \Comment{Update $r$}
            \State{Sample minibatch $\{x_m, \lambda_m \}_{m=1}^M$} of size $M$;
        ...
        \EndFor
        \State{Sample minibatch $\{x_m, y_m, \lambda_m \}_{m=1}^M$} of size $M$; \Comment{Update $f$}
        \State{With $\theta_r$ fixed, update $f$ by descending its stochastic gradient $\nabla_{\theta_f} E(\theta_f, \theta_r) :=$
        $$\nabla_{\theta_f}  \sum_{m=1}^M \left[ -\sum_{y_c \in {\cal Y}} 1(y_m = y_c) \log f(x_m;\theta_f)_c  +\sum_{\lambda_l \in \Lambda} 1(\lambda_m = \lambda_l)\log r(f(x_m;\theta_f);\theta_r)_l \right];$$}
    \EndFor
\end{algorithmic}
\end{algorithm}
...
\theta_r)$. If $r(f(X;\theta_f) ; \hat\theta_r )_l = \frac{1}{|\Lambda|}$
...
true conditional distribution of the nuisance $p_{\lambda|f(X;\theta_f)}$ and
...
\theta_r)$ are therefore such that $p_{\lambda|f(X;\theta_f)} =
p_{r(f(X;\theta_f);\hat\theta_r)|f(X)}$.

In other words, for all $\lambda_l \in  \Lambda$, we have $r(f(X;\theta_f);\hat\theta_r)_l = p(\lambda_l|f(X;\theta_f))$.
By assumption, $r(f(X;\theta_f);\hat\theta_r)_l = \frac{1}{|\Lambda|}$,
and therefore $p(\lambda_l|f(X;\theta_f)) = \frac{1}{|\Lambda|}$.
Using the Bayes' rule, we write
\begin{align*}
    p(f(X;\theta_f)|\lambda_l) &= \frac{ p(\lambda_l|f(X;\theta_f)) p(f(X;\theta_f)) } { p(\lambda_l)} \\
                               &= \frac{ \tfrac{1}{|\Lambda|} p(f(X;\theta_f)) } { \tfrac{1}{|\Lambda|} } \\
                               &= p(f(X;\theta_f)),
\end{align*}
which holds for all $\lambda_l \in \Lambda$ and implies that $f$ is a pivotal quantity.
\end{proof}
