...
\begin{figure*}
    ...
    \vspace{-1cm}
    \caption{Toy example.
    (Top left) Probability densities of the decision scores for $Z=0$, $2.5$ and $5$
    ...
    (Bottom left) Probability densities of the decision scores for $Z=0$, $2.5$ and $5$ when $f$ is
...
\begin{figure}
    ...
    \vspace{-1cm}
    \caption{Toy example. Training curves for $L_f(\theta_f)$, $L_r(\theta_f, \theta_r)$ and $L_f(\theta_f) - \lambda L_r(\theta_f, \theta_r)$.}
\end{figure}
...
generating training data $\{ x_i, y_i, z_i \}_{i=1}^N$, from which we build
a classifier $f$ minimizing $L_f(\theta_f)$ without adversarial training.
\glnote{to be continued}
