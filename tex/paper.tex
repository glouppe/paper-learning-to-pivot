...
its effectiveness  with a toy and a particle physics examples.
...
optimal.
When the lower bound is strict, let us note however that there may
exist distinct but equally good solutions $\theta_f,\theta_r$ minimizing Eqn.~\ref{eqn:vf-lambda}.
In this zero-sum game, an increase in accuracy would exactly be compensated by a decrease in pivotality and vice-versa.
How to best navigate this Pareto frontier to maximize a higher-level objective remains a question open for future works.
...
of five gaussians. Similar to $f$, the network architecture of $r$ comprises 2 dense hidden
...
and  relativistic effects will cause the two jets from its decay to
merge into a single `$W$-jet'.
These  $W$-jets have a rich internal substructure (see e.g.
...
In that sense, and to the best of our knowledge, our work is also original in the theoretical characterization
of the minimax solution of Eqn.~\ref{eqn:min_thetaf}, as well as in how one can trade-off accuracy for robustness through Eqn.~\ref{eqn:vf-lambda}.
