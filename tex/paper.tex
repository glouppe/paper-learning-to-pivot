...
parameter, as shown in the lower plots of Figure~\ref{fig:toy}. In particular,
the conditional probability densities of the decision scores $f(X;\theta_f)$ are
now very similar to each other, indicating only a small  dependency on the
nuisance, as theoretically expected. The dynamics of adversarial training is
illustrated in Figure~\ref{fig:toy-training}, where the losses ${\cal L}_f$,
...
are more accurate, hence decreasing ${\cal L}_f$, or that are less dependent on
