...
abuse of notation, $H(Z)$ denotes the differential entropy in this latter
...
$E(\hat\theta_f, \hat\theta_r) = \mathbb{E}_{X} [H({Y|X}) - H(Z)]$, then
...
$\mathbb{E}_{X} [ H({Z|f(X;\theta_f)}) ]$ of the conditional distribution of the nuisance. The
value function $E$ can therefore be rewritten as $$E'(\theta_f) = {\cal
L}_f(\theta_f) - \mathbb{E}_{X} [ H({Z|f(X;\theta_f)}) ].$$  In particular, we have the lower
bound $$\mathbb{E}_{X} [ H({Y|X}) - H(Z)] \leq {\cal L}_f(\theta_f) - \mathbb{E}_{X} [H({Z|f(X;\theta_f)})]$$
...
\begin{itemize}
    ...
    minimum value $\mathbb{E}_{X} [H({Y|X})]$,

    \item $\smash{\hat\theta}_f$ maximizes the expected entropy
    $\mathbb{E}_{X} [H({Z|f(X;\theta_f)})]$, which happens when all outcomes of
...
updated to improve ${\cal L}_f(\theta_f) - \mathbb{E}_X [ H({Z|f(X;\theta_f)}) ]$, then $f$
...
in which cases the lower bound $\mathbb{E}_{X} [ H({Y|X}) - H(Z)] < {\cal L}_f(\theta_f) - \mathbb{E}_{X} [H({Z|f(X;\theta_f)})]$ is strict: $f$ can either be an optimal classifier or a
...
\begin{figure}
    ...
    \caption{Toy example. Training curves for ${\cal L}_f(\theta_f)$, ${\cal L}_r(\theta_f, \theta_r)$
             ...
             Adversarial training was performed for $100$ iterations, mini-batches of size $M=128$, $K=100$ and $\lambda=50$.}
...
implemented as a mixture density network modeling $Z|f(X;\theta_f)$ as a
 mixture of five gaussians. As for $f$, the network
architecture comprises 2 dense hidden layers of 20 nodes with ReLU activations,
but is followed by an output layer of 15 nodes corresponding to the means,
standard deviations and mixture coefficients of the five gaussians. Output nodes
for the mean values come with linear activations, nodes
...
Similarly,  ${\cal L}_r$ keeps tending towards the differential entropy $H(Z) =
\log(\sigma \sqrt{2 \pi e}) = 1.419$ of the gaussian distribution with unit variance, as shown by the dashed
...
{\cal L}_r$ and the dashed line approximating $\mathbb{E}_{X} [ H({Y|X}) -
\lambda H(Z)]$.
