...
\begin{algorithmic}[1]
    \For{$t=1$ to $T$}
        \For{$k=1$ to $K$} \Comment{Update $r$}
            ...
            \State{With $\theta_f$ fixed, update the adversary $r$ by ascending its stochastic gradient
            $$\nabla_{\theta_r} \sum_{m=1}^M \left[ -\sum_{l=1}^{|\Lambda|} 1(\lambda_m = \lambda_l)\log r(f(x_m;\theta_f);\theta_r)_l \right];$$}
        \EndFor
        ...
        $$\nabla_{\theta_f} \sum_{m=1}^M \left[ \sum_{c=1}^{|{\cal Y}|} 1(y_m = y_c) \log f(x_m;\theta_f)_c -\sum_{l=1}^{|\Lambda|} 1(\lambda_m = \lambda_l)\log r(f(x_m;\theta_f);\theta_r)_l \right];$$}
    \EndFor
\end{algorithmic}
\end{algorithm}
