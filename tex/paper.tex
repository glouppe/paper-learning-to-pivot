...
Propositions below are derived in a non-parametric setting, by assuming that
both $f$ and $r$ have enough capacity. Results hold for a nuisance parameter $Z$
taking either categorical values or continuous values within a bounded support.
By abuse of notation, $H(p_Z)$ denotes the differential entropy in this latter
case. Finally, by construction, we require the uniform prior $p(z)$ over the
values $z \in {\cal Z}$ of the nuisance parameter.
...
not exist, in which case the lower bound $H(p_{Y|X}) - H(p_Z) < {\cal
L}_f(\theta_f) - H(p_{Z|f(X;\theta_f)})$ is strict: $f$ can either be an optimal
classifier or a pivotal quantity, but not both simultaneously. Accordingly the value
function $E$ can be rewritten as
\begin{equation}
...
value will preferrably enforces $f$ to be pivotal while setting $\lambda$ to a value close to $0$
