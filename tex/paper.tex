...
of the analysis to signals from potential new physics sources. \glnote{Shall we
be more explicit somewhere and put a formula underlining the effect of having a
pivotal classifier on the resulting sensitivity? (versus a classifier whose
decision function depends on the nuisance.)}
...
\begin{figure*}
    ...
    distribution $p(Z|f(X;\theta_f))$ of the nuisance as observed only through the output $f(X;\theta_f)$ of the classifier. By
    ...
    $f$ forces $p(Z|f(X;\theta_f)$ to become uniform, which happens
...
corresponding parameter $\theta^j$ of that distribution (e.g., the mean,
...
\begin{figure*}
    ...
    \vspace{-1cm}
    \caption{Toy example.
    (Upper left) Conditional probability densities of the decision scores for $Z=0, 1, \dots, 4$
       when $f$ is built without adversarial training. The resulting densities
       are clearly dependent on $Z$, indicating that $f$ is not pivotal.
    (Upper right) The corresponding decision surface, highlighting
       the fact that samples are easier to classify for values of $Z$ closer to $4$,
       hence partially explaining the dependency.
    (Lower left) Conditional probability densities of the decision scores for $Z=0, 1, \dots, 4$ when $f$ is
    ...
    (Lower right) The corresponding decision surface, illustrating how adversarial
       training bends the decision function to erase the dependency on $Z$.
    }
    \label{fig:toy}
\end{figure*}
...
\begin{figure}
    ...
    \caption{Toy example. Training curves for ${\cal L}_f(\theta_f)$, ${\cal L}_r(\theta_f, \theta_r)$
             ...
             Adversarial training was performed for $1000$ iterations, mini-batches of size $M=128$, $K=1$ and $\lambda=10$.}
...
\begin{align}
    x &\sim {\cal N}(\mu=(0,0), \sigma=2 \times \mathbb{1}) \text{ when } Y=0, \\
    x &\sim {\cal N}(\mu=(1,-1+z), \sigma=2 \times \mathbb{1}) \text{ when } Y=1.
\end{align}
...
Assuming a uniform prior over the interval ${\cal Z} = [0;4]$, we start by
generating training data $\{ x_i, y_i, z_i \}_{i=1}^N$, from which we build a
neural network classifier $f$ minimizing ${\cal L}_f(\theta_f)$ without adversarial
training. As shown in the upper plots of Figure~\ref{fig:toy}, the resulting
...
By contrast, when using Algorithm~\ref{alg:adversarial-training} for training
$f$ jointly with a mixture density network $r$ modeling the conditional
distribution of the nuisance as observed through $f$, the resulting classifier
becomes effectively almost independent on $Z$, as shown in the lower plots of
Figure~\ref{fig:toy}. In particular, the conditional probability densities of
the decision scores $f(X;\theta_f)$ are now very similar to each other,
indicating only a small  dependency on the nuisance, as theoretically expected.
The dynamics of adversarial training is illustrated in
...
that $f$ is worse than random because it rather tries to maximizes the
adversarial loss ${\cal L}_r$ by producing decision scores $f(X;\theta_f)$ that
are very unlikely under the current model $p_{\theta_r}$, at the expense of
making bad predictions with respect to the classification problem. As learning
goes and $p_{\theta_r}$ becomes more accurate, minimizing $E$ requires making
predictions that are more accurate, hence decreasing ${\cal L}_f$, or that are
less dependent on $Z$, hence shaping $p_{\theta_r}$ so that it becomes more
uniform. Indeed, ${\cal L}_f$ eventually start decreasing, while remaining lower
bounded by $\min_{\theta_f} {\cal L}_f(\theta_f)$ as approximated by the dashed
line in the first plot. Similarly,  ${\cal L}_r$ increasingly tends towards the
differential entropy $H(p_Z) = \log(4)$ of the uniform distribution over ${\cal
Z}$, as shown by the dashed line in the second plot. Finally, let us note that
the ideal situation of a classifier that is both optimal and pivotal appears to
be unreachable for this problem, as shown in third plot by the almost constant
offset between ${\cal L}_f - \lambda {\cal L}_r$ and the dashed line
approximating $\mathbb{E}_{X} [ H(p_{Y|X}) - \lambda H(p_Z)]$.

\glnote{Discuss practical considerations regarding the stability of training. A
better alternative seems to update $r$ until it is good
enough, then update $f$, and update $r$ again as soon as it becomes too bad.
This is important e.g. for MDN to prevent weights to explode if the network is
trained too much, and in which case gradients become useless.} \glnote{Warn against
setting $\lambda$ too high, since it could simply force $f$ to produce unlikely
...
To overcome this issue, the classifier $f$ is sometimes \glnote{add refs} built instead on a mixture
