...
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec in faucibus
lacus. Curabitur in viverra neque. Sed nec urna a sem blandit viverra vel a
eros. Nam a ultrices leo, a aliquet diam. Proin cursus scelerisque leo, ut
ullamcorper ante auctor et. Nulla odio nisl, molestie eget consectetur quis,
eleifend id ligula. Cras lobortis egestas justo ac sagittis. In at ligula est.
Nulla vel eros sapien. Sed bibendum metus quis dignissim scelerisque. Nulla arcu
lectus, ornare nec vehicula quis, lobortis a mi. Cras scelerisque mauris quam,
nec viverra ex convallis sit amet. Ut tellus turpis, lobortis sit amet finibus
sit amet, maximus ac est.
...
models the conditional distribution $p(z|f(X;\theta_f))$, e.g. with a network
whose 2 outputs are the parameters of a truncated gaussian and optimized such that
$p(z|f(X;\theta_f)) \approx N(\mu(f(X;\theta_f);\theta_r), \sigma(f(X;\theta_f);
\theta_r)$. The associated loss should be the negative log-likelihood (= cross-entropy), which is maximal when the
...
\glnote{This section and theoretical results should be rewritten in such a way
that categorical and continuous nuisance values are solved under the same
principled approach.}
