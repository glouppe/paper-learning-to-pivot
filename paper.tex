...
classes) minimizing  a loss ${\cal L}_f(\theta_f)$, with the constraint that $f(X_\lambda ;
...
and producing as output a prediction $\smash{\hat\lambda} = r(f(X_\lambda); \theta_f)$
of the nuisance parameter. If $f(X_\lambda; \theta_f)$ varies with $\lambda$,
then the corresponding correlation can be captured by $r$. By contrast, if
$f(X_\lambda; \theta_f)$ is invariant with $\lambda$ as we require, then $r$
